...
<table style="width:100%;border-collapse:collapse;">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="padding:0.75rem;">
                <?php
                // ถ้า icon เป็น SVG เต็มแท็ก ให้แสดงเลย
                if (preg_match('/^<svg/i', $cat['icon'])) {
                    echo $cat['icon'];
                } else {
                    // fallback: แสดง use จากชื่อไอคอน
                    echo '<svg width="24" height="24"><use href="#icon-' . htmlspecialchars($cat['icon']) . '"></use></svg>';
                }
                ?>
            </td>
            ...
            <td style="padding:0.75rem;text-align:right;">
                <?php
                    $jsData = json_encode([
                        ...
                        'color' => $cat['color'],
                        'icon' => $cat['icon']
                    ], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_HEX_QUOT|JSON_HEX_APOS);
                ?>
                <button onclick='openCategoryModal("edit", <?= htmlspecialchars($jsData, ENT_QUOTES) ?>)' class="adminit-btn" style="background:#eab308;color:#fff;padding:0.3rem 0.8rem;border-radius:6px;border:none;cursor:pointer;">แก้ไข</button>
        ...
        <form id="categoryForm" class="category-modal-form" method="POST">
            ...
            <div class="category-form-group">
                ...
                <div class="icon-preview">
                    ...
                    <span id="selectedIconName">custom</span>
                </div>
                
                <!-- ช่องให้ paste SVG -->
                <div style="margin-top:10px;">
                    <label class="category-form-label">วาง SVG icon ที่นี่:</label>
                    <textarea id="svgInput" style="width:100%;height:60px;resize:vertical;" placeholder="Paste SVG code here"></textarea>
                    <button type="button" class="category-modal-btn category-btn-secondary" style="margin-top:5px;" onclick="selectIcon(document.getElementById('svgInput').value)">ใช้ SVG นี้</button>
                </div>
                
                <!-- เลือกไอคอนจากชุดระบบ -->
                <div style="margin-top:15px;">
                    <label class="category-form-label">เลือกไอคอนจากชุดระบบ:</label>
                    <div id="iconPicker" style="display:grid;grid-template-columns:repeat(6, 1fr);gap:8px;background:#334155;padding:10px;border-radius:6px;margin-top:8px;">
                        <div class="icon-option" title="folder" onclick="pickSystemIcon('folder')"><svg width="20" height="20" fill="currentColor"><use href="#icon-folder"></use></svg></div>
                        <div class="icon-option" title="star" onclick="pickSystemIcon('star')"><svg width="20" height="20" fill="currentColor"><use href="#icon-star"></use></svg></div>
                        <div class="icon-option" title="heart" onclick="pickSystemIcon('heart')"><svg width="20" height="20" fill="currentColor"><use href="#icon-heart"></use></svg></div>
                        <div class="icon-option" title="tag" onclick="pickSystemIcon('tag')"><svg width="20" height="20" fill="currentColor"><use href="#icon-tag"></use></svg></div>
                        <div class="icon-option" title="bookmark" onclick="pickSystemIcon('bookmark')"><svg width="20" height="20" fill="currentColor"><use href="#icon-bookmark"></use></svg></div>
                        <div class="icon-option" title="document" onclick="pickSystemIcon('document')"><svg width="20" height="20" fill="currentColor"><use href="#icon-document"></use></svg></div>
                        <div class="icon-option" title="archive" onclick="pickSystemIcon('archive')"><svg width="20" height="20" fill="currentColor"><use href="#icon-archive"></use></svg></div>
                        <div class="icon-option" title="clipboard" onclick="pickSystemIcon('clipboard')"><svg width="20" height="20" fill="currentColor"><use href="#icon-clipboard"></use></svg></div>
                        <div class="icon-option" title="home" onclick="pickSystemIcon('home')"><svg width="20" height="20" fill="currentColor"><use href="#icon-home"></use></svg></div>
                        <div class="icon-option" title="user" onclick="pickSystemIcon('user')"><svg width="20" height="20" fill="currentColor"><use href="#icon-user"></use></svg></div>
                        <div class="icon-option" title="settings" onclick="pickSystemIcon('settings')"><svg width="20" height="20" fill="currentColor"><use href="#icon-settings"></use></svg></div>
                        <div class="icon-option" title="bell" onclick="pickSystemIcon('bell')"><svg width="20" height="20" fill="currentColor"><use href="#icon-bell"></use></svg></div>
                        <div class="icon-option" title="calendar" onclick="pickSystemIcon('calendar')"><svg width="20" height="20" fill="currentColor"><use href="#icon-calendar"></use></svg></div>
                        <div class="icon-option" title="chat" onclick="pickSystemIcon('chat')"><svg width="20" height="20" fill="currentColor"><use href="#icon-chat"></use></svg></div>
                        <div class="icon-option" title="cloud" onclick="pickSystemIcon('cloud')"><svg width="20" height="20" fill="currentColor"><use href="#icon-cloud"></use></svg></div>
                        <div class="icon-option" title="lock" onclick="pickSystemIcon('lock')"><svg width="20" height="20" fill="currentColor"><use href="#icon-lock"></use></svg></div>
                        <div class="icon-option" title="search" onclick="pickSystemIcon('search')"><svg width="20" height="20" fill="currentColor"><use href="#icon-search"></use></svg></div>
                        <div class="icon-option" title="wifi" onclick="pickSystemIcon('wifi')"><svg width="20" height="20" fill="currentColor"><use href="#icon-wifi"></use></svg></div>
                        <div class="icon-option" title="printer" onclick="pickSystemIcon('printer')"><svg width="20" height="20" fill="currentColor"><use href="#icon-printer"></use></svg></div>
                        <div class="icon-option" title="mail" onclick="pickSystemIcon('mail')"><svg width="20" height="20" fill="currentColor"><use href="#icon-mail"></use></svg></div>
                        <div class="icon-option" title="power" onclick="pickSystemIcon('power')"><svg width="20" height="20" fill="currentColor"><use href="#icon-power"></use></svg></div>
                        <div class="icon-option" title="smartphone" onclick="pickSystemIcon('smartphone')"><svg width="20" height="20" fill="currentColor"><use href="#icon-smartphone"></use></svg></div>
                        <div class="icon-option" title="shield" onclick="pickSystemIcon('shield')"><svg width="20" height="20" fill="currentColor"><use href="#icon-shield"></use></svg></div>
                        <div class="icon-option" title="cog" onclick="pickSystemIcon('cog')"><svg width="20" height="20" fill="currentColor"><use href="#icon-cog"></use></svg></div>
                        <div class="icon-option" title="wrench" onclick="pickSystemIcon('wrench')"><svg width="20" height="20" fill="currentColor"><use href="#icon-wrench"></use></svg></div>
                        <div class="icon-option" title="trash" onclick="pickSystemIcon('trash')"><svg width="20" height="20" fill="currentColor"><use href="#icon-trash"></use></svg></div>
                        <div class="icon-option" title="save" onclick="pickSystemIcon('save')"><svg width="20" height="20" fill="currentColor"><use href="#icon-save"></use></svg></div>
                        <div class="icon-option" title="edit" onclick="pickSystemIcon('edit')"><svg width="20" height="20" fill="currentColor"><use href="#icon-edit"></use></svg></div>
                        <div class="icon-option" title="plus" onclick="pickSystemIcon('plus')"><svg width="20" height="20" fill="currentColor"><use href="#icon-plus"></use></svg></div>
                        <div class="icon-option" title="x" onclick="pickSystemIcon('x')"><svg width="20" height="20" fill="currentColor"><use href="#icon-x"></use></svg></div>
                        <div class="icon-option" title="database" onclick="pickSystemIcon('database')"><svg width="20" height="20" fill="currentColor"><use href="#icon-database"></use></svg></div>
                    </div>
                </div>
            </div>
...
function selectIcon(iconValue) {
    ...
    let iconLabel = 'custom';
    if (iconValue.startsWith('<svg')) {
        iconLabel = 'custom';
    } else {
        iconLabel = iconValue;
    }
    document.getElementById('selectedIconName').textContent = iconLabel;
}
