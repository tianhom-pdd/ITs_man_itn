...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_child_node'])) {
    ...
    if ($child_title && $title_id) {
        try {
            ...
            $stmt->bind_param('sssii', $child_title, $child_description, $image_path, $title_id, $parent_id);
            $result = $stmt->execute();
            ...
            header("Location: editor.php?title_id=$title_id&parent_id=$current_parent_id&success=" . urlencode("เพิ่ม Node ลูกสำเร็จ"));
            exit;
