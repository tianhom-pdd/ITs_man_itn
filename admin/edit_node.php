...
$nodes = [];
$res = $db->query("SELECT id, title FROM cause_it WHERE id != $id ORDER BY title ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $title_id) {
        $stmt = $db->prepare("UPDATE cause_it SET title=?, type=?, image=?, title_id=?, parent_id=? WHERE id=?");
        $stmt->bind_param('sssiii', $title, $type, $image, $title_id, $parent_id, $id);
        $stmt->execute();
        header('Location: editor.php');
        exit;
    } else {
...
<h1 style="margin-bottom:2rem;">แก้ไข Node</h1>
<?php if (!empty($error)): ?>
    ...
    <div style="margin-bottom:1.2rem;">
        ...
        <select name="title_id" id="title_id" class="adminit-input" style="width:100%;padding:0.5rem;" required>
            ...
            <?php foreach($titles as $cat): ?>
                <option value="<?= $cat['id'] ?>" <?= $cat['id']==$node['title_id']?'selected':'' ?>><?= htmlspecialchars($cat['title']) ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div style="margin-bottom:1.2rem;">
        <label for="title" style="display:block; margin-bottom:0.5rem;">ชื่อ Node <span style="color:#dc2626">*</span></label>
        <input type="text" name="title" id="title" class="adminit-input" style="width:100%;padding:0.5rem;" value="<?= htmlspecialchars($node['title']) ?>" required>
    </div>
    <div style="margin-bottom:1.2rem;">
        <label for="type" style="display:block; margin-bottom:0.5rem;">ประเภท</label>
        <input type="text" name="type" id="type" class="adminit-input" style="width:100%;padding:0.5rem;" value="<?= htmlspecialchars($node['type']) ?>">
    </div>
    <div style="margin-bottom:1.2rem;">
        ...
        <select name="parent_id" id="parent_id" class="adminit-input" style="width:100%;padding:0.5rem;">
            ...
            <?php foreach($nodes as $n): ?>
                <option value="<?= $n['id'] ?>" <?= $n['id']==$node['parent_id']?'selected':'' ?>><?= htmlspecialchars($n['title']) ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div style="margin-bottom:1.2rem;">
        ...
        <?php if($node['image']): ?>
            <div style="margin-bottom:0.5rem;">
                <img src="../uploads/<?= htmlspecialchars($node['image']) ?>" alt="img" style="max-width:60px;max-height:60px;">
            </div>
        <?php endif; ?>
        <input type="file" name="image" id="image" accept="image/*">
    </div>
    <div>
        <button type="submit" class="adminit-btn" style="background:#eab308;color:#fff;padding:0.5rem 1.5rem;border-radius:6px;">บันทึก</button>
